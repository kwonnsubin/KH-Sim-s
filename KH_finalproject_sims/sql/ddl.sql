...
CREATE TABLE BIZ (
	...
	BIZ_OWNER_NAME	VARCHAR2(15)	NULL,
...
ALTER TABLE BIZ MODIFY (BIZ_REVIEW_STAR NUMBER DEFAULT 0);
...
ALTER TABLE QNA_ANS ADD CONSTRAINT FK_QNA_TO_ANS FOREIGN KEY (AQ_NO) REFERENCES QNA(AQ_NO) ON DELETE CASCADE;
...
ALTER TABLE QNA_RPL ADD CONSTRAINT FK_ANS_TO_RPL FOREIGN KEY (AA_NO) REFERENCES QNA_ANS(AA_NO) ON DELETE CASCADE;
...
-- 요금제 가입정보 테이블 생성
DROP TABLE PLAN_ORDER CASCADE CONSTRAINTS;
CREATE TABLE PLAN_ORDER(
    ...
    CURRENT_TELECOM CHAR(1) CHECK(CURRENT_TELECOM IN (1,2,3,4,5,6)),
...
ALTER TABLE PLAN_ORDER ADD CONSTRAINT PLAN_ORDER_SIM_YN_CHECK CHECK(SIM_YN IN (1,2));
ALTER TABLE plan_order ADD biz_id varchar2(30);
ALTER TABLE plan_order ADD CONSTRAINT fk_biz_id2 FOREIGN KEY (biz_id) REFERENCES biz(biz_id);
...
ALTER TABLE PAYINFO_CARD ADD CONSTRAINT FK_ORDER_TO_CARD FOREIGN KEY (ORDER_NO) REFERENCES PLAN_ORDER (ORDER_NO);
ALTER TABLE PAYINFO_CARD ADD CONSTRAINT CARD_RELATIONSHIP_CHECK CHECK(CARD_RELATIONSHIP IN (1,2,3,4));

--- 계좌납부정보 테이블
DROP TABLE PAYINFO_ACC;
...
ALTER TABLE PAYINFO_ACC ADD CONSTRAINT FK_ORDER_TO_ACC FOREIGN KEY (ORDER_NO) REFERENCES PLAN_ORDER (ORDER_NO);
...
CREATE TABLE CUSTOM_Q (
    ...
    CQ_SPEED NUMBER CHECK(CQ_SPEED IN (1,2,4)),
    CQ_VOICE NUMBER CHECK(CQ_VOICE IN (1,2,3,4))
);
ALTER TABLE CUSTOM_Q ADD CONSTRAINT FK_USER_TO_CQ FOREIGN KEY (USER_ID) REFERENCES USER_TABLE (USER_ID);
...
ALTER TABLE VIEW_TABLE ADD CONSTRAINT FK_USER_TO_VIEW FOREIGN KEY (USER_ID) REFERENCES USER_TABLE (USER_ID);
ALTER TABLE VIEW_TABLE ADD CONSTRAINT FK_PLAN_TO_VIEW FOREIGN KEY (PLAN_NO) REFERENCES PLAN_TABLE (PLAN_NO);
