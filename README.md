...
1. 약관에 모두 동의하지 않으면 다음으로 넘어갈 수 없음
    1-1. 약관에 모두 동의 체크하면 모든 항목에 체크 표시
    1-2. 모든 동의 항목에 체크하면 모두 동의에 체크 표시
...
6. 카드납부 혹은 계좌납부를 선택하면 신청한 납부 수단의 정보를 작성할 수 있는 입력폼이 나타남
    6-1. 
...
2. 가입상태에 따라 페이지 분기 (신청중,반려,탈퇴 - 신청 페이지(applyDetail) / 승인완료- 상세정보 페이지(bizDetail)<br>
    2-1. divCheck 변수 선언<br> 
    2-2. case문 사용하여 화면으로 이동할 때 가져가는 값 다르게 설정)<br>
